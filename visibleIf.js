...
var visibleIf = new function(){
    ...
    var visibleIfNodes;
    var mandatoryNodes;
    var varRe = /\s([a-zA-Z][a-zA-Z0-9\.\_\[\]]*)\s/g;
    ...
    function setEvents(){
        ...
        for (var i = 0; i < forms.length; i++) {
            $(forms[i]).submit(function() {formSubmitEvent()});
        }
                                ...
                                if (fieldNode != null) {
                                    if (fieldNode.nodeName != "SELECT" && fieldNode.length) {
                                        for (var k = 0; k < fieldNode.length; k++) {
                                            $(fieldNode[k]).click(function() {refreshPageEvent()});
                                        }
                                    } else {
                                        $(fieldNode).change(function() {refreshPageEvent()});
                                    }
                                    ...
                                    if (fieldNode.type == 'number') {
                                        //click when clicking the controls, 
                                        $(fieldNode).click(function() {refreshPageEvent()});
                                        //change when entering text, not working in current browsers (4.2012)
                                        $(fieldNode).change(function() {refreshPageEvent()});
                                    }
	...
	function updateVisibilityProperties(formNode){
		...
		for (var i = 0; i < fields.length; i++) {
			var field = fields[i]
            if (CSSHelpers.isMemberOfClass(field, 'visibleIf')){
...
$(document).ready(function() {visibleIf.init()});
